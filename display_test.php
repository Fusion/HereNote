...
$result_list = $db->query("SELECT title,slug FROM blog_blogpost where title like '%" . 'test' . "%' OR content like '%" . 'test' . "%'");
while($row = $result_list->fetchArray()) {
    $results['posts'][] = array('slug' => $row['slug'], 'title' => $row['title']);
}
$result_list = $db->query("SELECT titles,slug FROM pages_page left join pages_richtextpage on id=page_ptr_id where content like '%" . 'test' . "%'");
while($row = $result_list->fetchArray()) {
