...
if(!$user->can_edit) {
    ...
    if(!empty($_POST['Save'])) {
        ...
        if($slug == 'new_rich' || $slug == 'new_markdown') {
            ...
            $stmt = $db->prepare("UPDATE mae_posts SET title=:title, content=:content, description=:description WHERE section=2 AND slug=:slug");
