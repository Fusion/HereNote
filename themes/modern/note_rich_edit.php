...
<form id='edit_all' action='/note/<?=$this->get('slug')?>/edit/' method='post'>
<fieldset>
    ...
    <div>
        ...
        <div>
            ...
            <div class="display_more-content">
                ...
                <input style="width:100%" name="title" id="note_title" value="<?=$this->get('title')?>" />
            </div>
        </div>
    </div>
    <div> &nbsp; </div>
    <input id="note_id" type="hidden" name="id" value="<?=$this->get('id')?>" />
...
jQuery('#edit_all').submit(function(e) {
    if(jQuery('#note_title').val() == '') {
        e.preventDefault();
        inform('Empty field', 'You need to provide a title for this note', 'error');
    }
    else {
